\documentclass[aspectratio=169, 10pt]{beamer}

% === PREÁMBULO ===
\usepackage[utf8]{inputenc}
\usepackage[spanish]{babel}
\usepackage{graphicx}
\usepackage{amsmath}
\usepackage{amssymb}
\usepackage{tikz}
\usetikzlibrary{positioning}
\usepackage{booktabs}
\usepackage{tcolorbox}

% === PALETA DE COLORES ===
\definecolor{C1}{RGB}{20, 80, 60}       % verde oscuro — estructura
\definecolor{C2}{RGB}{46, 139, 104}     % verde medio — acento
\definecolor{C3}{RGB}{210, 240, 225}    % verde muy claro — fondo bloque
\definecolor{CA}{RGB}{180, 100, 20}     % naranja — alerta / énfasis
\definecolor{CG}{RGB}{245, 245, 242}    % gris cálido — fondo slide
\definecolor{CT}{RGB}{30, 30, 30}       % texto principal

% === TEMA Y COLORES ===
\usetheme{Madrid}           % Otros: Antibes, Bergen, Berlin, CambridgeUS, Copenhagen, etc.
\usecolortheme{default}     % Otros: albatross, beaver, beetle, crane, dolphin, dove, etc.

% === CONFIGURAR COLORES BEAMER ===
\setbeamercolor{normal text}{fg=CT, bg=CG}
\setbeamercolor{frametitle}{fg=white, bg=C1}
\setbeamercolor{structure}{fg=C1}
\setbeamercolor{block title}{bg=C1, fg=white}
\setbeamercolor{block body}{bg=C3}
\setbeamercolor{block title alerted}{bg=CA, fg=white}
\setbeamercolor{block body alerted}{bg=CA!12}
\setbeamercolor{block title example}{bg=C2, fg=white}
\setbeamercolor{block body example}{bg=C2!15}
\setbeamertemplate{itemize item}{\color{C2}\textbullet}
\setbeamertemplate{itemize subitem}{\color{C2}$\circ$}

% === ELIMINAR BARRA DE NAVEGACIÓN ===
\setbeamertemplate{navigation symbols}{}

% === TAMAÑOS DE FUENTES ===
\setbeamerfont{frametitle}{size=\large, series=\bfseries}
\setbeamerfont{title}{size=\Large, series=\bfseries}
\setbeamerfont{block title}{series=\bfseries, size=\small}
\setbeamerfont{block body}{size=\small}
\setbeamerfont{itemize/enumerate body}{size=\normalsize}

% === INFORMACIÓN DEL DOCUMENTO ===
\title{Mi Primera Presentación}
\subtitle{Un template básico de Beamer}
\author{Tu Nombre}
\institute{Tu Institución}
\date{\today}

% === LOGO (OPCIONAL) ===
% \logo{\includegraphics[height=1.5cm]{logo.png}}

\begin{document}

% ============================================
% DIAPOSITIVA DE TÍTULO
% ============================================
\frame{\titlepage}

% ============================================
% TABLA DE CONTENIDOS
% ============================================
\begin{frame}{Contenido}
    \tableofcontents
\end{frame}

% ============================================
% SECCIÓN 1: DIAPOSITIVAS BÁSICAS
% ============================================
\section{Diapositivas Básicas}

\begin{frame}{Diapositiva Simple}



\end{frame}

\begin{frame}{El problema: Medir la geometría de los poros}
    \begin{columns}[T]
        \column{0.5\textwidth}

        \begin{block}{Material: AAO}
            Membrana nanoestructurada con poros cilíndricos organizados en red hexagonal. Sus propiedades ópticas y mecánicas dependen de la geometría de los poros.
        \end{block}

        \begin{block}{Objetivo}
            Predecir automáticamente los parámetros geométricos de una imagen SEM.
        \end{block}

        \begin{alertblock}{5 variables objetivo}
            \renewcommand{\arraystretch}{1.25}
            \begin{tabular}{@{}lll@{}}
              $R$,\;$\sigma_R$       & Radio promedio y dispersión [px] \\
              $DCC$,\;$\sigma_{DCC}$ & Distancia centro a centro [px] \\
              $P$                    & Porosidad (\%)                  \\
            \end{tabular}
        \end{alertblock}


        \column{0.5\textwidth}

        \begin{center}
            \includegraphics[width=0.7\textwidth]{Imagenes/ImagenAAO.png}
        \end{center}

    \end{columns}
\end{frame}

% ============================================
% SECCIÓN 2: Generación del Dataset Simulado
% ============================================

\section{Generación del Dataset Simulado}

\begin{frame}{Generación del Dataset Simulado}



\begin{columns}[t] % alineación arriba
    \column{0.32\textwidth}

% ---- COLUMNA IZQUIERDA (tabla) ----

    \begin{block}{Parámetros aleatorios por imagen}

        \renewcommand{\arraystretch}{1.2}
        \begin{tabular}{@{}ll@{}}
        \toprule
        Parámetro & Rango \\
        \midrule
        Radio $r$              & $[10,\;30]$ px \\
        Distancia $dcc$        & $[2.5r,\;4.5r]$ \\
        Desorden en $r$, $dcc$ & hasta 10\% \\
        Rotación de red        & $[0^\circ,\;360^\circ]$ \\
        Brillo $B$             & $[-40,\;40]$ \\
        Contraste $C$          & $[0.6,\;1.4]$ \\
        Ruido                  & nivel 1 \\
        \bottomrule
        \end{tabular}

    \end{block} 

    \begin{center}
        \begin{tcolorbox}[
        colback=C3,
        colframe=C2,
        boxrule=1pt,
        arc=6pt,              % redondeo de bordes
        width=1.5\linewidth, % ancho del box
        left=6pt, right=6pt,
        top=4pt, bottom=4pt,
        boxsep=2pt,
        center                % centra el box
        ]
        \small
        Se generaron \textbf{20\,000 imágenes} de $300\times300$ px. Cada imagen genera una fila en el CSV con variables: 
        $R$, $\sigma_R$, $DCC$, $\sigma_{DCC}$, $P$.
        \end{tcolorbox}        
    \end{center}



% ---- COLUMNA DERECHA (imágenes) ----
\column{0.45\textwidth}
    \centering

    \includegraphics[width=0.45\textwidth]{Imagenes/ImagenAAO.png}
    \includegraphics[width=0.45\textwidth]{Imagenes/ImagenAAO.png}

    \vspace{0.3cm}

    \includegraphics[width=0.45\textwidth]{Imagenes/ImagenAAO.png}
    \includegraphics[width=0.45\textwidth]{Imagenes/ImagenAAO.png}

\end{columns}

   


\end{frame}



\section{Bloques y Cuadros}



% ============================================
% SECCIÓN 3: Normalización de las labels
% ============================================
\section{Normalización de las labels}

\begin{frame}{Normalización de las labels}
    \begin{columns}[T]
        \column{0.5\textwidth}
        \begin{block}{¿Por qué normalizar las labels?}
            Las variables tienen escalas muy distintas: $R \sim 20$ \, px pero  $P \sim 0.3$.Sin normalizar, las variables grandes dominan el MAE y el modelo aprende mal.
        \end{block}

        \begin{exampleblock}{StandardScaler}
            Técnica de normalización que transforma las variables:
            $$
                x_{norm} = \frac{x - \mu}{\sigma}
            $$
            donde $\mu$ es la media y $\sigma$ es la desviación estándar.
            
            \textbf{Resultado:} variables con media 0 y desviación estándar 1.
        \end{exampleblock}
        
        \column{0.5\textwidth}
    
        \begin{center}
            \begin{tikzpicture}[node distance=1cm, >=stealth]
                
                % Nodo 1
                \node[draw, rounded corners, fill=C3, minimum width=3cm, minimum height=0.7cm] (inicio)
                    {CSV Original $\rightarrow$ ($R$, $\sigma_R$, $DCC$, $\sigma_{DCC}$, $P$)};
                
                % Nodo 2
                \node[draw, rounded corners, fill=C2!30, minimum width=3cm, minimum height=0.7cm, below=of inicio] (paso1)
                    {Paso 1};
                
                % Nodo 3
                \node[draw, rounded corners, fill=C2!30, minimum width=3cm, minimum height=0.7cm, below=of paso1] (paso2)
                    {Paso 2};
                
                % Nodo 4
                \node[draw, rounded corners, fill=CA!30, minimum width=3cm, minimum height=0.7cm, below=of paso2] (fin)
                    {Fin};
                
                % Flechas
                \draw[->, thick, color=C2] (inicio) -- (paso1);
                \draw[->, thick, color=C2] (paso1) -- (paso2);
                \draw[->, thick, color=C2] (paso2) -- (fin);
                
            \end{tikzpicture}
        \end{center}
    \end{columns}
\end{frame}

% ============================================
% SECCIÓN 4: MATEMÁTICAS
% ============================================
\section{Ecuaciones}

\begin{frame}{Ecuaciones Matemáticas}
    Una ecuación inline: $E = mc^2$
    
    Una ecuación en modo destaque:
    \[
        \int_0^{\infty} e^{-x^2} dx = \frac{\sqrt{\pi}}{2}
    \]
    
    Sistema de ecuaciones:
    \begin{align*}
        a + b &= c \\
        d - e &= f
    \end{align*}
\end{frame}

% ============================================
% SECCIÓN 5: IMÁGENES Y FIGURAS
% ============================================
\section{Figuras}

\begin{frame}{Diagrama de Flujo Simple}

    \begin{center}
        \begin{tikzpicture}[node distance=1.5cm, >=stealth]
            
            % Nodo 1
            \node[draw, rounded corners, fill=C3, minimum width=3cm, minimum height=0.7cm] (inicio)
                {Inicio};
            
            % Nodo 2
            \node[draw, rounded corners, fill=C2!30, minimum width=3cm, minimum height=0.7cm, below=of inicio] (paso1)
                {Paso 1};
            
            % Nodo 3
            \node[draw, rounded corners, fill=C2!30, minimum width=3cm, minimum height=0.7cm, below=of paso1] (paso2)
                {Paso 2};
            
            % Nodo 4
            \node[draw, rounded corners, fill=CA!30, minimum width=3cm, minimum height=0.7cm, below=of paso2] (fin)
                {Fin};
            
            % Flechas
            \draw[->, thick, color=C2] (inicio) -- (paso1);
            \draw[->, thick, color=C2] (paso1) -- (paso2);
            \draw[->, thick, color=C2] (paso2) -- (fin);
            
        \end{tikzpicture}
    \end{center}

\end{frame}

% ============================================
% SECCIÓN 6: LISTAS NUMERADAS Y CÓDIGO
% ============================================
\section{Listas Numeradas}

\begin{frame}{Listas Numeradas}
    Pasos a seguir:
    \begin{enumerate}
        \item Primer paso
        \item Segundo paso
        \item Tercer paso
    \end{enumerate}
\end{frame}

% ============================================
% DIAPOSITIVA FINAL
% ============================================
\begin{frame}[plain]
    \begin{center}
        {\Large \textbf{¡Gracias!}}
        
        \vspace{1cm}
        
        Preguntas e inquietudes
    \end{center}
\end{frame}

\end{document}
